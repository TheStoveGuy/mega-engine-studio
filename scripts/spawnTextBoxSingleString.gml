...
linelength = 29; //despite the number in the textbox object itself being 28, it actually works as 29
...
while (string_length(mytext) > 0)
{
    ...
    if (ep > 0 && (newLinePos <= 0 || ep < newLinePos)) //only count effects if before new line break
    {
        //account for all of the text effects in the word, only saving the last one
        while (string_length(e) > 0 && ep > 0)
        {
            ep = string_pos("/C", e); //it always gives the position of the first instance
    ...
    if (ep > 0 && (newLinePos <= 0 || ep < newLinePos)) //only count effects if before new line break
    {
        ...
        while (string_length(e) > 0 && ep > 0)
        {
            ep = string_pos("/", e); //it always gives the position of the first instance
    ...
    subtext = string_delete(subtext, 1, s); //remove the word form the text to check
    
    //checking newLinePos because if there is a line break command and this is also true, it'll overwrite the line break task
    if (newLinePos <= 0 && string_length(mytext) - effectSpace - 1 <= linelength) //if the remaining text fits into a single line
    {
        space = linelength * 2; //trigger adding the rest of the text to the current line
        prespace = string_length(mytext); //set the end of the line to save as the end of the entire text
    }
    
    
    
    //if the line is finished, add to the array of lines for the current page
    if (space - effectSpace - 1 > linelength) //-1 so we don't count the space or line break as part of the word
    {
        t[z] = string_copy(mytext, 1 - 1, prespace - 1); //cut off the space / line break
        ...
        mytext = string_delete(mytext, 1, prespace); //delete the line that was just saved from the remaining text
        subtext = mytext;
        space = 0;
        effectSpace = 0;
        prespace = 0;
        ...
        if (z == 5 or string_length(mytext) <= 0)
        {
            if (is_string(txt[i]))
            {
            ...
            if (i = 13) {break;} //if reached max textbox pages, then don't do any more pages
    ...
    if (timeOut >= 1000)
    {
        show_error("An infinite loop occured in the text wrap parsing! Here's the status of the variables.
space: " + string(space) + "
prespace: " + string(prespace) + "
...
if (timeOut < 1000)
{
    Spawntextbox(0, 0, _name,
    txt[1], txt[2], txt[3], txt[4], txt[5], txt[6], txt[7],
    txt[8], txt[9], txt[10], txt[11], txt[12], txt[13]);
}
