...
var t,_w,_h,_x,_y,_minx,_miny,_maxx,_maxy;
t = argument[0];
if argument_count > 1 {
    _w = argument[1];
    _h = argument[2];
...
tw = tile_get_width(t)
th = tile_get_height(t)
tx = tile_get_x(t);
ty = tile_get_y(t);
tl = tile_get_left(t);
tt = tile_get_top(t)
td = tile_get_depth(t);
tsrc = tile_get_background(t);
...
for (_x = floor(tx/_w)*_w;_x <= ceil((tx+tw)/_w)*_w; _x += _w) {
    for (_y = floor(ty/_h)*_h;_y <= ceil((ty+th)/_h)*_h; _y += _h) {
        ...
        tile_add(tsrc,_minx - tx + tl,_miny - ty + tt,_maxx - _minx, _maxy - _miny, _minx, _miny, td)
    }
}
